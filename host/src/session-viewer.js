 ...
 *
 * Usage: session-viewer.js <port> <code> <token>
 * The token is a random nonce generated by the host for authentication.
 */
...
if (!port || isNaN(port) || port < 1 || port > 65535) {
  console.error('Usage: session-viewer.js <port> <code> <token>\nPort must be a number between 1 and 65535.');
  process.exit(1);
}

if (!token) {
  console.error('Missing viewer token. Cannot authenticate.');
  process.exit(1);
}
...
const client = net.connect({ port, host: '127.0.0.1' }, () => {
  ...
  // Send the authentication token as the very first message
  client.write(token + '\n');
